...
const err = {
  ...
  isUndefined: (paramName = "<unknown parameter>", param: string) => {
    if ([null, undefined].indexOf(param) > -1)
      err.throwError(err.getCaller(2), paramName);
  },

  isFalsy: (paramName = "<unknown parameter>", param: string) => {
  ...
  isNotType: (paramName = "<unknown parameter>", param: string, type = "") => {
    if (getType(param) !== type.toLowerCase())
      err.throwError(
        err.getCaller(2),
        paramName,
        "to be type " + type.toLowerCase(),
      );
  },
  ...
  containsUndefined: (paramName = "<unknown parameter>", param: string) => {
  ...
  isInvalidPath: (paramName = "<unknown parameter>", param: string) => {
    err.isUndefined(paramName, param);
  ...
  isInvalidWriteData: (paramName = "<unknown parameter>", param: string) => {
    err.isUndefined(paramName, param);
